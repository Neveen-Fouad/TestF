...
if (requireLogin()) {
  ...
  profileForm.addEventListener("submit", async event => { event.preventDefault(); const values = Object.fromEntries(new FormData(profileForm)); try { const [profileResult, settingsResult] = await Promise.all([api.profile.update({ first_name: values.first_name, last_name: values.last_name, email: values.email }), api.dashboard.updateSettings({ phone: values.phone, birth_date: values.birth_date })]); const user = profileResult?.data?.profile || profileResult?.data || profileResult; const settings = settingsResult?.data?.profileSettings || {}; session.updateUser(user); fill({ ...settings, ...user }); profileForm.hidden = true; notify("Profile saved."); } catch (error) { notify(error.message, true); } });
