...
if (requireLogin()) {
  ...
  const passwordForm = document.querySelector("#password-form"); const pw = passwordForm.elements.password; const pwConfirm = passwordForm.elements.password_confirmation; function validatePasswordMatch() { pwConfirm.setCustomValidity(pwConfirm.value && pw.value !== pwConfirm.value ? "Passwords do not match." : ""); } pw.addEventListener("input", validatePasswordMatch); pwConfirm.addEventListener("input", validatePasswordMatch); passwordForm.addEventListener("submit", async event => { event.preventDefault(); if (!confirm("Are you sure you want to change your password? You will need to sign in again.")) return; const values = Object.fromEntries(new FormData(event.currentTarget)); try { await api.profile.password(values); event.currentTarget.reset(); notify("Password updated."); } catch (error) { notify(error.message, true); } });
}
